// JS는 함수에 정의 매개변수보다 전달된 인자가 많아도 오류가 바랭 x
function sum(a, b) {
  return a + b;
}
...
function sumTotal() {
  ...
  let result = 0;
  argArr.forEach((num) => {
    result += num; // 배열을 돌면서 각 숫자를 result에 산술 할당 => 배열의 reduce 대체 가능
  });
...
// 참조 복사 :객체를 할당하면 복사되는 것이 라니라 주소값이 할당된다.
//          => 내용을 변경하면 원복 객체가 변한다.
let copy = obj;
copy.age = 41;
...
console.log(obj === copy02); // obj와 copy02 다른 객체 이다.
...
// 비구조화 할당 (destructuring) : 배열이나 객체의 값을 변수에 할당할 때 간단하게 표련할 수 있다.
let { name: userName, age, foo } = obj; //객체 비구조화 할당. {}안에 객체의 키값을 넣으면 해당 속성값을 같은 이름의 변수로 할당한다.
console.log(userName, age, foo);

let fuirts = ["onange", "apple", "banana"];
let [frist, second, third] = fuirts; // 배열 비구조화 할당. let [] 안에 변수이름들 자유롭게 지으면 배열의 순서대로 할당한다.
